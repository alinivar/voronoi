...
#define pallete_count (sizeof(pallete)/sizeof(pallete[0]))
...
void render_voronoi(void)
{
    for (int y = 0; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) {
            ...
            image[y][x] = pallete[j % pallete_count];
        }
    }
}
...
void save_image_as_ppm(const char* file_path)
{
    ...
    if (f == NULL) {
        fprintf(stderr, "ERROR: Couldn't write into file %s: %s\n", file_path, strerror(errno));
        exit(1);
    }
